...
router.delete('/Master/Delete/:userId', async (req, res) => {
    ...
    // SQL 쿼리 초기화
    let deleteUserSql, deleteStoresSql, deleteMasterAlertsSql;
    ...
    try {
        ...
        if (member_jic === 'guard') {
            ...
            console.log("Users에서 Guard 삭제 완료");

            res.send({ message: 'Guard가 성공적으로 삭제되었습니다.' });
        } else if (member_jic === 'admin') {
            // Admin 관련 데이터 삭제
            deleteStoresSql = 'DELETE FROM Stores WHERE user_id = ?';
            deleteUserSql = 'DELETE FROM Users WHERE user_id = ?';
            ...
            await db.executeQuery(deleteUserSql, [userId]);
            console.log("Users에서 Admin 삭제 완료");
            ...
            await db.executeQuery(deleteUserSql, [userId]);
            console.log("Users에서 User 삭제 완료");
...
router.post("/push/update/:user_id", async (req, res) => {
  ...
  try {
    ...
    if (result && result.affectedRows > 0) {
      res
        .status(200)
        .send({ message: "푸시 알림 설정이 성공적으로 업데이트되었습니다." });
    } else {
      res.status(404).send({ error: "사용자를 찾을 수 없습니다." });
    }
  } catch (err) {
